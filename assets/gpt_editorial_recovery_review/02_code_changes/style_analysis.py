...
class HumanStyleAnalyzer(BaseStyleAnalyzer):
    ...
    def analyze(
        ...
        formulaic_mix_score = clamp_unit(
            (0.4 if mechanism_mix_profile.unique_group_type_count >= 3 else 0.0)
            + (0.25 * min(1.0, surface_wordplay_group_count / 2.0))
            + (0.2 * microtheme_smallness)
            + (0.15 * (1.0 - earned_wordplay_score))
            + (0.1 if "formulaic_mixed_template" in editorial_metadata["editorial_flags"] else 0.0)
        )
        editorial_payoff_score = clamp_unit(
            (0.35 * label_naturalness_score)
            + (0.3 * evidence_interpretability)
            + (
                0.2
                * max(
                    earned_wordplay_score,
                    0.4 if not earned_wordplay_scores else earned_wordplay_score,
                )
            )
            + (0.15 * (1.0 - microtheme_smallness))
        )
        family_repetition_risk = clamp_unit(
            (0.45 * formulaic_mix_score)
            + (0.25 * repeated_pattern_family_score)
            + (0.3 * microtheme_smallness)
        )
        editorial_flatness_score = clamp_unit(
            (0.45 * formulaic_mix_score)
            + (0.3 * surface_wordplay_score)
            + (0.15 * (1.0 - label_naturalness_score))
            + (0.1 * (1.0 - editorial_payoff_score))
        )
        weak_label_naturalness_score = clamp_unit(1.0 - label_naturalness_score)
        family_saturation = clamp_unit(
            (0.6 * family_repetition_risk) + (0.4 * min(1.0, surface_wordplay_group_count / 2.0))
        )
        ...
        metrics = {
            ...
            "phrase_template_payoff_score": round(editorial_payoff_score, 4),
        }
        ...
        style_alignment_score = (
            round(mean(1.0 if comparison.within_band else 0.0 for comparison in comparisons), 4)
            if comparisons
            else 0.0
        )
