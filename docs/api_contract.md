...
## `GET /debug/evaluation/latest`

Returns the latest persisted batch-evaluation summary and top-k debug view if one exists.
...
- Batch evaluation artifacts are generated by `scripts/evaluate_batch.py` rather than through a write API endpoint.
